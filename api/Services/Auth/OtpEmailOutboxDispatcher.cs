...
public sealed class OtpEmailOutboxDispatcher(
    AppDbContext dbContext,
    IOtpEmailSender emailSender,
    IDataProtectionProvider dataProtectionProvider,
    IOptions<OtpOptions> options,
    TimeProvider timeProvider,
    ILogger<OtpEmailOutboxDispatcher> logger)
{
    public async Task<bool> DispatchAsync(Guid outboxId, CancellationToken cancellationToken = default)
    {
        ...
        var message = await dbContext.OtpEmailOutboxMessages
            .FirstOrDefaultAsync(entry => entry.Id == outboxId, cancellationToken);

        if (message is null)
        {
            logger.LogWarning("OTP email outbox message {OutboxId} was not found.", outboxId);
        ...
        try
        {
            plainOtpCode = OtpEmailOutboxPayload.Unprotect(dataProtectionProvider, message.ProtectedPayload);
        }
        ...
        try
        {
            await emailSender.SendOtpAsync(message.Email, plainOtpCode, message.Id, CancellationToken.None);
        }
